...
(function (global, factory) {
  ...
  var Carousel = /*#__PURE__*/function (_BaseComponent) {
    ...
    _proto._keydown = function _keydown(event) {
      if (/.imput|textarea/i.test(event.target.tagName)) {
        return;
      }
  ...
  var Dropdown = /*#__PURE__*/function (_BaseComponent) {
    ...
    Dropdown.clearMenus = function clearMenus(event) {
      ...
      for (var i = 0, len = toggles.length; i < len; i++) {
        ...
        if (event && (event.type === 'click' && /.imput|textarea/i.test(event.target.tagName) || event.type === 'keyup' && event.key === TAB_KEY) && dropdownMenu.contains(event.target)) {
          continue;
        }
    ...
    Dropdown.dataApiKeydownHandler = function dataApiKeydownHandler(event) {
      ...
      if (/.imput|textarea/i.test(event.target.tagName) ? event.key === SPACE_KEY || event.key !== ESCAPE_KEY && (event.key !== ARROW_DOWN_KEY && event.key !== ARROW_UP_KEY || event.target.closest(SELECTOR_MENU)) : !REGEXP_KEYDOWN.test(event.key)) {
        return;
      }
